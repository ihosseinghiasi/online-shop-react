...
const ShowAdmin = () => {
    ...
    async function getAdmin () {
        await axios.post('http://localhost:4000/adminPanel/admin/showAdmin', params).then((res) => {
                        ...
                        <form onSubmit={(event) => updateAdmin(event)}> 
                            <div className="row g-2" >
                                <div className="mx-4 col-5">  
                                    ...
                                    />
                                    <input type="text" className="form-control mt-3 faField" value={values.lastName}  placeholder="نام خانوادگی" name="lastName" 
                                        onChange={(event) => setValues({...values, [event.target.name]: event.target.value})}
                                    />
                                    <input type="email" className="form-control mt-3 enField" value={values.email}  placeholder="ایمیل" name="email" 
                                        onChange={(event) => setValues({...values, [event.target.name]: event.target.value})}
                                    />
                                    <select className="form-select mt-3 faField" value={values.department} name="department"
                                        onChange={(event) => setValues({...values, [event.target.name]: event.target.value})}
                                    >
                                        <option selected>مدیریت</option>
                                        <option>پشتیبانی</option>
                                    </select>
                                    <input type="password" className="form-control mt-3 enField" value={values.password}
                                ...
                                <div className="mx-3 col-6">
                                        ...
                                        <div className="form-check form-switch float-end my-1 me-5">
                                            <input type="checkbox" className="btn-check" id="chkAdmin" name="isAdmin" checked={values.isAdmin}
                                        ...
                                        <div className="form-check form-switch float-end my-1 me-2">
                                            <input type="checkbox" className="btn-check" id="chkProduct" name="isProduct" checked={values.isProduct}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                            <label className="btn btn-outline-success faField" for="chkProduct">محصولات</label>                                        
                                        </div>
                                        <div className="form-check form-switch float-end my-1 me-2">
                                            <input type="checkbox" className="btn-check" id="chkCard" name="isCard" checked={values.isCard}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                            <label className="btn btn-outline-success faField" for="chkCard">کارت ها</label>                                        
                                        </div>
                                        <div className="form-check form-switch float-end my-1 me-5">
                                            <input type="checkbox" className="btn-check" id="chkEmail" name="isEmail" checked={values.isEmail}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                            <label className="btn btn-outline-success faField" for="chkEmail">ایمیل ها</label>                                        
                                        </div>
                                        <div className="form-check form-switch float-end my-1 me-2">
                                            <input type="checkbox" className="btn-check" id="chkReport" name="isReport" checked={values.isReport}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                            <label className="btn btn-outline-success faField" for="chkReport">گزارشات</label>                                        
                                        </div>
                                        <div className="form-check form-switch float-end my-1 me-2">
                                            <input type="checkbox" className="btn-check" id="chkTicket" name="isTicket" checked={values.isTicket}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                            <label className="btn btn-outline-success faField" for="chkTicket">تیکت ها</label>                                        
                                        </div>
                                        <div className="form-check form-switch float-end my-1 me-5">
                                            <input type="checkbox" className="btn-check" id="chkCategory" name="isCategory" checked={values.isCategory}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                            <label className="btn btn-outline-success faField" for="chkCategory">دسته بندی ها</label>                                        
                                        </div>
                                        <div className="form-check form-switch float-end my-1 me-2">
                                            <input type="checkbox" className="btn-check" id="chkUser" name="isUser" checked={values.isUser}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
                                        ...
                                        <div className="form-check form-switch float-end mt-1 mb-5 me-2">
                                            <input type="checkbox" className="btn-check" id="chkPayment" name="isPayment" checked={values.isPayment}
                                                onChange={(event) => setValues({...values, [event.target.name]: event.target.checked})} />
